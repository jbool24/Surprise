...
exports.create = function(request, response) {
    ...
    function valid(user) {
        Session.createSessionForUser(user, false, function(err, sess, authyResponse) {
            if (err || !sess) {
                error(response, 500, 'Error creating session - please log in again.');
            } else {
                // Send the unique token for this session and the onetouch response
                response.send({token: sess.token, authyResponse: authyResponse});
            }
        });
    }
};
...
exports.authyStatus = function(request, response) {
    var status = (request.user)
        ? request.user.authyStatus
        : 'unverified';
    if (status == 'approved') {
        request.session.confirmed = true;
        request.session.save(function(err) {
            if (err)
                return error(response, 500, 'There was an error validating your session.');
            }
        );
    }
    if (!request.session) {
...
exports.verify = function(request, response) {
    ...
    request.user.verifyAuthyToken(oneTimeCode, function(err) {
        if (err)
            return error(response, 401, 'Invalid confirmation code.');
...
exports.resend = function(request, response) {
    if (!request.user)
        return error(response, 404, 'No user found for this session, please log in again.');

    // Otherwise resend the code
    request.user.sendAuthyToken(function(err) {
        if (!request.user)
            return error(response, 500, 'No user found for this session, please log in again.');
