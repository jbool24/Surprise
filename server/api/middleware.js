...
exports.loadUser = function(request, response, next) {
    ...
    Session.findOne({ token: token }, function(err, doc) {
        if (err || !doc)
            return next();
...
exports.loginRequired = function(request, response, next) {
    if (!request.session || !request.session.confirmed) {
        response.status(403).render('login' , { error: 'Your session has expired - please log in again.'});
    } else {
      next();
    }
};
...
exports.validateSignature = function(request, response, next) {
    var key = process.env.AUTHY_API_KEY;
    ...
    if (theirs != mine) {
        response.status(401).send({status: 401, message: "This request is unsigned."});
    } else {
        next();
    }
};
